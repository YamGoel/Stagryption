...
##### FOR IMAGE USING ChaCha20Poly1305 ####
import image
...
class NIS_Project:
    ...
    def chacha20_decrypt_audio():
        # Generate a random 256-bit key using PBKDF2 with a provided password and a salt
        salt = b'\x00' * 16  # You should use the same salt used during encryption
        kdf = PBKDF2HMAC(
            algorithm=hashes.SHA256(),
            length=32,
            salt=salt,
        ...
        nonce = b'\x00' * 16  # You should use the same nonce used during encryption

        # Read the encrypted audio data
        with open("encrypted_audio.m4a", 'rb') as file:
            ciphertext = file.read()

        # Create a ChaCha20-Poly1305 cipher instance with the key
        cipher = Cipher(
            algorithms.ChaCha20(key, nonce),
            mode=None,
            backend=default_backend()
    ...
    def main(self, root):
        ...
        def new_encryption_frame(root):
            ...
            def new_audio_frame(encryption_frame):
                ...
                # subAudio = tk.Button(audio_frame, text="Submit", command=lambda: subAudio(audio_frame))
                # subAudio.config(font=('Times new roman', 10, 'bold'))
                # subAudio.pack(padx=10, pady=10)
    ...
    def performSteganography(encrypted_data, data): 
        ...
        if not myfile:
            ...
            # o_image_w, o_image_h = my_img.size
            # board.pack()
            # encode_button = tk.Button(steganography, text='Cancel', command=lambda : IMG_Stegno.back(self,steganography))
